...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		DirectX::Get().OnSize(ResizeWidth, ResizeHeight);
		return 0;
	case WM_SYSCOMMAND:
	...
	if (FHGUI::Input::Get().OnWindowProc(hWnd, message, wParam, lParam))
		return 0;
...
bool MainWindow::Init(int Width, int Height)
{
	...
	WndClass_.lpszClassName = L"FlowHooks";
	RegisterClassEx(&WndClass_);

	hWnd_ = CreateWindow(WndClass_.lpszClassName, L"FlowHooks GUI Showcase", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, Width, Height, nullptr, nullptr, WndClass_.hInstance, nullptr);
